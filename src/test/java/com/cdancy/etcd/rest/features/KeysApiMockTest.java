...
public class KeysApiMockTest extends BaseEtcdMockTest {
    ...
    public void testCreateKeyWithTTL() throws Exception {
        ...
        try {
            ...
            assertTrue(createdKey.node().key().equals("/hello"));
            assertTrue(createdKey.node().value().equals("world"));
            assertSentWithFormData(server, "PUT", "/" + EtcdApiMetadata.API_VERSION + "/keys/hello",
                    "value=world&ttl=5");
        } finally {
            etcdApi.close();
            server.shutdown();
    ...
    public void testListInOrderKey() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(payloadFromResource("/keys-create-in-order-list.json"))
                .setResponseCode(200));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
    ...
    public void testListInOrderKeyNonExistent() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(payloadFromResource("/keys-dir-delete-nonexistent.json"))
                .setResponseCode(404));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
    ...
    public void testGetNonExistentKey() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(payloadFromResource("/keys-get-delete-nonexistent.json"))
                .setResponseCode(404));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
    ...
    public void testCompareAndSwapKeyValue() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(payloadFromResource("/keys-compare-and-swap-value.json"))
                .setResponseCode(200));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
    ...
    public void testCompareAndSwapKeyExist() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(payloadFromResource("/keys-compare-and-swap-exist.json"))
                .setResponseCode(200));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
    ...
    public void testCreateDirWithTTL() throws Exception {
        ...
        server.enqueue(
                new MockResponse().setBody(payloadFromResource("/keys-dir-create-ttl.json")).setResponseCode(201));
        EtcdApi etcdApi = api(server.getUrl("/"));
        KeysApi api = etcdApi.keysApi();
        try {
