 ...
 */
package com.cdancy.etcd.rest.filters;
...
@Singleton
public class EtcdAuthentication implements HttpRequestFilter {
   private final Supplier<Credentials> creds;

   private final String REGEX = "^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)$";

   @Inject
   EtcdAuthentication(@Provider Supplier<Credentials> creds) {
      this.creds = creds;
   }
